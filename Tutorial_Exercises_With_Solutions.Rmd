...
fake_news<-data_ML %>% filter(is_fake==1)
tic()
mycorpus<-future({tokens(fake_news$text, 
             remove_punct = TRUE, # this removes punctuation
             remove_numbers = TRUE, # this removes digits
             remove_symbol = TRUE)%>% 
  tokens_remove(pattern = stopwords("en", source = "marimo"))}) 
...
## Conclusions Exercise 2: "Iterate over multiple inputs with Purrr and Furrr" 

With the above example we managed to decrease the execution time by using the Furrr package. However, it should be noted that it was only 1.5 times faster than using Purrr. This can be explained by Amdahl's Law, which states that there are diminishing returns in parallelization, depending on the proportion of your code that can be executed in parallel.
...
This example shows that the time it takes to do the 10,000-iteration simulation is approximately 4 times faster when using parallel implementation with the package Future.apply. This example can also be evaluated with the package Furrr and we would have similar results. Furthermore, we will
analyze the CPU history in our laptops to see what is happening while using Future package.
